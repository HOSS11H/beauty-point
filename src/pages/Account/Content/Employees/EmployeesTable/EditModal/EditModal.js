...
const EditModal = (props) => {
    ...
    const { show, heading, confirmText, onConfirm, onClose, id, fetchedEmployees, fetchedLocations } = props;
    const { t } = useTranslation();
    const themeCtx = useContext(ThemeContext)

    const selectedProductIndex = fetchedEmployees.data.findIndex(product => product.id === id);

    let productData = fetchedEmployees.data[selectedProductIndex];
